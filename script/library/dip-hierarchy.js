...
Hierarchy.prototype = {
  ...
  recursiveDirectory : function(node, html, indentIndex) {
    for(var i=0; i<node.getChildren().length; i++) {
      html += "<div class='node_wrap' id ='dir_" + node.getChildren()[i].getId() + "'>";
  ...
  setDirectoryClickListener : function() {
    console.log("!!!!!!!");
    this.recursiveSetClickListener(this.getRoot());
  },

  recursiveSetClickListener : function(node) {
    for(var i=0; i<node.getChildren().length; i++) {
      var nodeId = node.getChildren()[i].getId();

      $("#dir_" + nodeId).click(function(event) {
        var eventId = $(this).attr('id').split("_")[1];

        showAttrs(eventId);
        dip.getProject()[dip.getIndexById(dip.getCurProjectId())].setSelectedComponent(eventId *= 1);
      });

      if(node.getChildren()[i].getChildren().length > 0) {
        this.recursiveSetClickListener(node.getChildren()[i]);
      }
    }
  },
}
