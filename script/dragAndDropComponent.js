  ...
  $(".draggable_tools").draggable({
    ...
    stop: function(event, ui) {
      if(dip.getProject().length) {
        ...
        dip.getProject()[dip.getIndexById(dip.getCurProjectId())].getHierarchy().setDirectoryClickListener();
      }
    }
  });
}
...
function onDragAndDropComponentInWorkspace() {
  $(".draggable_workspace").draggable({
    ...
    stop: function(event, ui) {
      var eventId = $(this).attr('id');
      if(dip.getProject()[dip.getIndexById(dip.getCurProjectId())].getHierarchy().getNode(eventId).getData().getDataType() == 'text') {
        var textValue = $('#' + eventId + " textarea").val();
        console.log("onDragAndDropComponentInWorkspace -> textValue : " + textValue);
...
function initComponentEvent(id) {
  console.log("initComponentEvent -> id : " + id);
